...
EDOtrans <-
  function(Data, Cls, Means, SDs, Weights, DO = FALSE, PlotIt = FALSE, KS = FALSE, Criterion = "BIC",
           Razor = TRUE, Gap = FALSE, GapOnly = FALSE, MaxModes = 10, MaxCores = 28, Seed, PlotGMM = FALSE)
   {
    ...
    if (hasArg("Means") == TRUE &
        hasArg("SDs") == TRUE & hasArg("Weights") == TRUE) {
      ...
      if (length(Means0) > 1) {
        Boundaries <-
          AdaptGauss::BayesDecisionBoundaries(Means = Means0, SDs = SDs0, Weights = Weights0)
        if (is.integer0(Boundaries) == FALSE)
          Boundaries <-
            Boundaries[Boundaries >= min(Means0) & Boundaries <= max(Means0)]
      ...
      if (hasArg("Cls") == TRUE) {
        if (length(Cls) != length(Data)) {
          ...
          Weights0 <-
            tapply(X = Data, INDEX = Cls, function(x)
              length(x) / length(Data))
        }
      } else {
    ...
    if (length(Weights0) > 1) {
      ...
      if (is.integer0(WeightsABC$Aind) == FALSE) {
        ...
        CombinedDominatGroupsParameters <-
          combinedModesParameters(
            Means = Means0,
            SDs = SDs0,
            Weights = Weights0,
            n = nDom
