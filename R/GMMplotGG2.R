#Function to plot a Gaussian mixture based on the data and/or on the model's parameter values
...
#' @import ggplot2
#' @export
GMMplotGG2 <- function(Data, Cls, Means, SDs, Weights, BayesBoundaries,
                       SingleGausses = TRUE, Hist = FALSE, Dotplot = FALSE, Bounds = TRUE, SumModes = TRUE, PDE = TRUE) {
  if (hasArg(Cls) == TRUE) {
    if (length(Cls) != length(Data))
      stop("GMMplotGGdens: Unequal legths of Data and Cls.")
  }
  if (hasArg(Cls) == FALSE) {
    Cls <- rep(1, length(Data))
    warning("GMMplotGGdens: Classes not provided. Assuming one class only.", call. = FALSE)
  }
  if (hasArg("Means") == TRUE | hasArg("SDs") == TRUE | hasArg("Weights") == TRUE) {
    if (!hasArg("Means")  | !hasArg("SDs") | !hasArg("Weights")) {
      stop("GMMplotGG: Incomplete GMM parameters.")
    } else {
      if (length(c(Means, SDs, Weights)) %% 3 != 0)
        stop("GMMplotGG: Unequal number of modes in parameters.")
    }
  } else {
    Means <- tapply(X = Data, INDEX =  Cls, FUN = mean)
    SDs <- tapply(X = Data, INDEX =  Cls, FUN = sd)
    Weights <- tapply(X = Data, INDEX =  Cls, function(x) length(x) / length(Data))
    warning("GMMplotGG: Means, SDs and Weights per class calculated from data.", call. = FALSE)
  }
  ...
  if (Hist == TRUE | Dotplot == TRUE) {
    ...
    if (Hist == TRUE) {
      p1 <- p1 + geom_histogram(data = data.frame(HistData), aes(x = HistData, .data$..density..),
                                binwidth = bwidth, fill = "grey85", color = "grey95") +
        guides(fill = FALSE)
    }
    if (Dotplot == TRUE) {
      p1 <- p1 +
        ylim(c(ggplot_build(p1)$layout$panel_params[[1]]$y.range[1], max(SumPDF$y))) +
        geom_dotplot(data = data.frame(HistData), aes(x = HistData, fill = (factor(Cls))),
